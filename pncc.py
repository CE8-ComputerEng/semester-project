...
def pncc(audio_wave, n_fft=512, sr=16000, winlen=0.020, winstep=0.010,
    ...
    # Use scipy dct instead of librosa dct
    #dct = scipy.fftpack.dct(power_law_nonlinearity, axis=1, norm='ortho')[:, :n_pncc]
    dct = np.dot(power_law_nonlinearity, scipy.fftpack.dct(
        np.eye(power_law_nonlinearity.shape[1]), axis=0, norm='ortho').T)[:, :n_pncc] 
    return dct